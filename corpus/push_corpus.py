...
async def _load_file(file_name: str) -> list[dict]:
    ...
    for section in sections:
        ...
        header: list[str] = re.findall(r"<header>(.*?)</header>", section, re.DOTALL)
        context: list[str] = re.findall(r"<context>(.*?)</context>", section, re.DOTALL)
        document: list[str] = re.findall(r"<document>(.*?)</document>", section, re.DOTALL)
