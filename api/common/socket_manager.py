...
from api.common.schemas import MetaData, SocketResponse
...
class SocketManager:
	...
	async def send_message(
		...
		response = SocketResponse(
			metadata=MetaData(success=success, message=message),
			type=type,
			data=data,
		)
	...
	async def close_on_error(self):
		await self.ws.close(code=1006, reason='Error occurred')
